...
public class AppraisalVisitor implements Visitor {
    ...
    public static class Ratings extends HashMap<Integer, PerformanceRating> {
    ...
    private int getTeamAverage(Employee emp) {
        return (int) Math.round(emp.getDirectReports().stream().mapToDouble(Employee::getPerformanceRating).average().getAsDouble());
    }
